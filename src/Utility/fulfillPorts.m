...
% For any unconnected ports, creates a dummy block to connect it to and
% does so. Outports are connected to terminators and Inports are connected
% to grounds. Unconnected ports are probably indicative of a
% problem so this function shouldn't be used haphazardly as it may hide
% problems.
%
%   Input
...
for i = 1:length(ports)
    ...
    if strcmp(get_param(ports(i),'PortType'),'outport')
        ...
        if ~hasDst
            % Create terminator
            bHandle = add_block('built-in/Terminator', [portSys '/generated_terminator'], 'MakeNameUnique', 'on');
