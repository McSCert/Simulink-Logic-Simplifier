function newExprs = substituteExpressions(exprs, blocks, predicates)
...
for i = length(newExprs):-1:1
    if allowedToSubIntoOthers(lefts{i}, newExprs{i}, newExprs, SUBSYSTEM_RULE)
        ...
        for j = 1:length(newExprs)
            if allowedToBeSubbedInto(lefts{j}, exprBlocks{j}, newExprs{i}, SUBSYSTEM_RULE)
