function swapBlockPattern(sys, blocks, extraSupport)
...
%%
% Make input blocks a cell array of block fullnames.
...
orBlock = {};
for z = 1:length(blocks)
    if strcmp(get_param(blocks{z}, 'BlockType'), 'Logic') ...
            && strcmp(get_param(blocks{z}, 'Operator'), 'OR')
        orBlock{end+1} = blocks{z};
    end
end
for z = 1:length(orBlock)
    ...
    andBlocks = {};
    for i = 1:length(ph) % For each source of the current or block
        parent = get_param(ph(i), 'Parent');
        if strcmp(get_param(parent, 'BlockType'), 'Logic') ...
                && strcmp(get_param(parent, 'Operator'), 'AND') ...
                && any(strcmp(parent, blocks)) % If not in blocks then not allowed to swap it
...
function swapBlockPattern_sys(sys, extraSupport)
% SWAPBLOCKPATTERN_SYS Swap Simulink blocks fitting a certain pattern for a
% simpler pattern. For example, logical AND and OR blocks can be connected
% in such a way that is equivalent to a Switch block thus this connection
% pattern will be recognized and the blocks will be replaced with a Switch.
%
