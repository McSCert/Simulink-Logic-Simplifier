function [finalExprs, baseExprs] = doSimplification(sys, blocks, varargin)
...
%                               simplification.
%       finalExprs       Expressions generated by simplifying the
...
postSimpleExprs = cell(1,length(preSimpleExprs));
for i = 1:length(preSimpleExprs)
    if ~isBlackBoxExpression(preSimpleExprs{i})
        [lhs, rhs] = getExpressionLhsRhs(preSimpleExprs{i});
...
%         rhs = strrep(rhs, 'CbFALSE', 'FALSE');
        ...
        postSimpleExprs{i} = [lhs, ' = ', simpleRhs];
    else
        postSimpleExprs{i} = preSimpleExprs{i};
    end
end
