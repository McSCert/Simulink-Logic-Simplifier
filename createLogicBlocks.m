...
%proposition names to signals.
    ...
    %this main loop functions as the parser of the expression. Loopes until the entire string is parsed.
    while (index <= length(expression))
        ...
        switch character
            ...
            otherwise
                ...
                num = regexp(atomic{1}, '^[0-9]+', 'match');
                if ~isempty(num)
                    num = num{1};
                    isConstant = (length(atomic{1}))==(length(num));
                else
                    isConstant = 0;
                end
                if ~isConstant
                    ...
                    if ~isKey(atomicExpr, num)
                        atomicBlock = add_block('built-in/Constant', [sys '/generated_constant_' num]);
                        set_param(atomicBlock, 'Value', num);
                        atomicExpr(num) = atomicBlock;
                        index = index + length(atomic{1});
                        subExpr = expression(startIndex:index);
                        memo(subExpr) = atomicBlock;
                    else
